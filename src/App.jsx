...
function ItemRow({ item, onCycle, onRemove, onRateRequest, onDescUpdate, theme: T, isArchive }) {
  ...
  return (
    <div style={{ background: T.bgCard, border: `1px solid ${done ? T.borderLight : T.border}`, borderRadius: 10, marginBottom: "0.4rem", overflow: "hidden", opacity: done ? 0.68 : 1, transition: "opacity 0.2s" }}>
...
function ListPage({ userId, listType, sections, theme: T }) {
  ...
  const doneItems = items.filter(i => i.status === doneStatus).sort((a, b) => (b.rating || 0) - (a.rating || 0));
